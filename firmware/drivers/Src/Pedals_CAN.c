...
PedalsStatus pedals_CAN_init() { 
	if(!MX_CAN_Init()) return PEDALS_CAN_INIT_FAIL;
	else return PEDALS_OK;
}

PedalsStatus pedals_CAN_start() { 
	if (can_start(hcan1) != CAN_OK) return PEDALS_CAN_START_FAIL;
    return PEDALS_OK;
}
...
static void potsV_FillPayload(uint8_t *tx_data, PedalsMsg msg) {
	//split 16 bit fixed point mV values to two 8 bit vals (little endian - LSB is 1st byte)
...
PedalsStatus pedals_CAN_send_PotsP(PedalsMsg msg, TickType_t delayTicks) {
	...
	if(can_send(hcan1, &tx_header, tx_data, delayTicks) != CAN_OK) return PEDALS_CAN_SEND_FAIL;
