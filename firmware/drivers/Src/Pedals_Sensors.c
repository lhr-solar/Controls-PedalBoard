...
adc_status_t adc_start_read(Pedals_ADC_Channel channel) {
...
static QueueHandle_t adc_get_queue(ADC_Input_t adc_input, const char **name,
...
Pedals_Status_t adc_receive(ADC_Input_t adc_input, uint32_t *val) {
	...
	QueueHandle_t tempRecvQ =
		adc_get_queue(adc_input, &name, &success, &fail);
